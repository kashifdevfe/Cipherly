...
import { AesMode, FormatType, KeySize, encryptAes, encodeFormat } from '@/lib/aes-crypto';
import { motion, AnimatePresence } from 'framer-motion';

export default function AesEncryptPanel() {
  ...
  const handleEncrypt = useCallback(async () => {
    ...
    if (mode !== 'ECB' && !iv) return setError('IV is required for this mode.');

    setError('');
    setIsLoading(true);
    
    try {
      const encryptedBytes = await encryptAes(
        text,
        key,
        keyFormat,
        keySize,
      ...
      const formattedResult = encodeFormat(encryptedBytes, outputFormat);
  ...
  }, [text, key, keyFormat, keySize, mode, iv, ivFormat, tagLength, outputFormat]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        {mode === 'ECB' && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] font-bold uppercase tracking-tight">
            <AlertCircle className="w-4 h-4 shrink-0" />
            Warning: ECB mode is insecure for structured data.
          </div>
        )}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-center items-center gap-2">
          ...
          <span className="text-[10px] font-medium text-muted-foreground">
            100% Browser-Native. Zero Data Leakage.
          </span>
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4 border-t border-border/50">
        <div className="flex items-center justify-between">
          <label className="text-sm font-semibold">Encrypted Ciphertext</label>
